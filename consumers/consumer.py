...
class KafkaConsumer:
    ...
    def _consume(self):
        ...
        try:
            ...
            print(f"Failed to deserialize message with error {err}")
            return 0

        if not message:
            return 0
        elif message.error():
            logger.error(f"Error retrieving message from Kafka topic {self.topic_name_pattern} with error: {message.error()}")
            return 0
        else:
